                    ...
                    @foreach($cargoBids as $cargoBid)
                        <div class="order-info">
                            ...
                            <div class="order-info-content">
                                ...
                                @if($cargoBid->initiator_id == auth()->user()->id || (!$cargoBid->initiator_id && $cargoBid->user_id == auth()->id()))
                                ...
                                @elseif($cargoBid->user_id == auth()->id() && $cargoBid->initiator_id)
                                    ...
                                    <div class="order-info__card order-info__buttom order-card-border">
                                        ...
                                        <div class="order-info-col order-info-bottom__end">
                                            ...
                                            <div class="order-buttons-second">
                                                <p>{{ __('lang.Грузовладелец предложил груз') }}</p>
                                                <div class="order-buttons">
                                                    <button class="form-btn button-text" data-modal-target="modal-create-order-{{ $cargoBid->id }}">{{ __('lang.Оформить') }}</button>

                                                    <button class="button-text chat-message" data-modal-target="dropdown-close-{{ $cargoBid->id }}">{{ __('lang.Отклонить') }}</button>

                                                    <span>{{ $cargoBid?->created_at?->format('d.m.Y H:i') }}</span>
                                                </div>

                                                <div class="order-cansel-modal order-cansel-dropdown" data-modal="dropdown-close-{{ $cargoBid->id }}">
                                                    ...
                                                    <form action="{{ route('cargo.bids.decline', [app()->getLocale(), $cargoBid?->id]) }}" method="POST">
                                                        @csrf
                                                        <textarea name="comment" data-error="{{ __('lang.Пожалуйста, укажите причину отмены.') }}" id="comment-{{ $cargoBid?->id }}" style="height: auto; overflow: hidden" required></textarea>

                                                        <label for="save-{{ $cargoBid?->id }}" hidden>
                                                            {{ __('lang.Восстановить груз') }}
                                                            <input type="radio" name="archiveOption" id="save-{{ $cargoBid?->id }}" value="restore" checked>
                                                        </label>

                                                        <button class="form-btn" onclick="submitDeclineForm({{ $cargoBid?->id }})" data-modal-close="dropdown-close-{{ $cargoBid->id }}">{{ __('lang.Отклонить предложение') }}</button>
                                                        <button type="button" class="order-cansel" data-modal-close="dropdown-close-{{ $cargoBid->id }}">{{ __('lang.Не отклонять') }}</button>
                                                    </form>
                                                </div>

                                                <div class="modal-overlay" data-modal="modal-create-order-{{ $cargoBid->id }}">
                                                    <div class="modal modal-create-order">
                                                        <b>{{ __('lang.Взять груз') }}</b>
                                                        ...
                                                        <form action="{{ route('cargo.bids.accept', [app()->getLocale(), $cargoBid->id]) }}" method="POST">
                                                            ...
                                                            <div class="create-order-card">
                                                                ...
                                                                <div class="create-order-card__info">
                                                                    <p>
                                                                        <strong>{{ $cargoBid->cargoLoading->cargo->title }}</strong>
                                                                    ...
                                                                    <div class="modal-row">
                                                                        <div class="road">
                                                                            ...
                                                                            {{ app('google.translator')->setTarget(app()->getLocale())->translate($cargoBid->cargoLoading->country) }}
                                                                            ...
                                                                            {{ app('google.translator')->setTarget(app()->getLocale())->translate($cargoBid->cargoLoading->final_unload_city) }}
                                                                        </div>
                                                                    </div>
                                                                    <br>
                                                                   <p><strong>{{ $cargoBid->cargoLoading->final_unload_date_from?->format('d.m.Y') }}</strong></p>
                                                                    @if($cargoBid->cargoLoading?->company?->phones->isNotEmpty())
                                                                    @foreach($cargoBid->cargoLoading?->company?->phones as $phone)
                                                                    <span><a href="tel:{{ str_replace(['(', ')', '-', ' '], '', $phone->phone) }}">{{ $phone->phone }}</a></span>
                                                                    @endforeach
                                                                    @endif
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="create-order-buttons">
                                                                <button class="form-btn" data-modal-close="modal-create-order-{{ $cargoBid->id }}">Взять груз</button>
                                                            </div>
                                                        </form>

                                                        <button class="modal-close" type="button" data-modal-close="modal-create-order-{{ $cargoBid->id }}">
                                                            <span></span>
                                                            <span></span>
                                                        </button>
                                                    </div>
                                        ...
                                        <div class="order-buttons-second">
                                            ...
                                            <div class="order-cansel-modal order-cansel-dropdown" data-modal="dropdown-close-{{ $cargoBid->id }}">
                                                ...
                                                <form action="{{ route('cargo.bids.decline', [app()->getLocale(), $cargoBid?->id]) }}" method="POST">
                                                    ...
                                                    <label for="save-{{ $cargoBid?->id }}">
                                                        {{ __('lang.Восстановить груз') }}
                                                        <input type="radio" name="archiveOption" id="save-{{ $cargoBid?->id }}" value="restore" checked>
                                                    </label>
                                                    <label for="stay-{{ $cargoBid?->id }}">
                                                        {{ __('lang.Оставить груз в архиве') }}
                                                        <input type="radio" name="archiveOption" id="stay-{{ $cargoBid?->id }}" value="keep">
                                                    </label>
                            ...
                            <div class="modal modal-create-order">
                                ...
                                <form action="{{ route('cargo.bids.accept', [app()->getLocale(), $cargoBid?->id]) }}" method="post">
                                    ...
                                    <div class="create-order-card">
                                        ...
                                        <div class="create-order-card__info">
                                            <p><strong>{{ $cargoBid?->user?->full_name ?? __('lang.Неизвестно') }}</strong></p>
                                            <p>{{ $cargoBid?->user?->phone }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="cretae-order-row">
                                        <p>
                                            ...
                                            <strong>{{ $cargoBid->price }} {{ $cargoBid->cargoLoading->currency }}</strong>
                                        </p>
                                    </div>
                                    @else
                                        ...
                                        @if($cargoBid->without_vat_cashless)
                                            <div class="cretae-order-row">
                                                <p>
                                                    <svg width="24" height="18">
                                                        <use xlink:href="#create1"></use>
                                        ...
                                        @endif
                                        @if($cargoBid->is_prepayment || $cargoBid->prepayment_percent)
                                            <div class="cretae-order-row">
                                                <p>
                                                    <strong>{{ __('lang.Предоплата:') }} {{ $cargoBid?->prepayment_percent }}%</strong>
                                                </p>
                                            </div>
                                        @endif

                                        @if($cargoBid->is_on_unloading || $cargoBid->is_bank_transfer || $cargoBid->bank_transfer_days)
                                            <div class="cretae-order-row">

                                                <p>
                                                    @if($cargoBid->is_on_unloading)
                                                        <strong>{{ __('lang.На выгрузке') }}</strong>
                                                    @endif

                                                    @if($cargoBid->is_on_unloading || $cargoBid->bank_transfer_days)
                                                    <strong>{{ __('lang.Через:') }}</strong> {{ $cargoBid->bank_transfer_days }} {{ __('lang.дней') }}
                                                    @endif
                                                </p>
                                            </div>
                                        @endif
                                    ...
                                    <div class="create-order-buttons">
                                        <button type="submit" class="form-btn" data-modal-close="modal-create-order-{{ $cargoBid->id }}">{{ __('lang.Одобрить и создать заказ') }}</button>
                                    </div>
                                </form>
